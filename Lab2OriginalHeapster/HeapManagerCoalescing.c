...
void my_free(void *data) {
    ...
    // Revision 1
    /*if (((char *)previous + BLOCK_SIZE + previous->block_size) == header) { // Yes, coalesce.
        printf("Coalescing once to the left!\n");
        ...
        while (previous->next_block != NULL) {
            if (((char *)previous + BLOCK_SIZE + previous->block_size) == previous->next_block) {
                printf("Coalescing other free blocks to the left!\n");
                previous->block_size += BLOCK_SIZE + header->block_size;
                previous->next_block = previous->next_block->next_block;
            }
        }
    }*/
    ...
    // Shove right?
    else if (((char *)header + BLOCK_SIZE + header->block_size) == next) {
        printf("Stage 1: Shove right.\n");
        ...
        // Shove left?
        /*if (((char *)previous + BLOCK_SIZE + previous->block_size) == header) {
            printf("Stage 2: Shove left.\n");
            previous->block_size += BLOCK_SIZE + header->block_size;
            previous->next_block = header->next_block;
         }
        my_free(0);
    }
    */
    if (previous != NULL && header != NULL && (((char *)previous + BLOCK_SIZE + previous->block_size) == header || ((char *)header + BLOCK_SIZE + header->block_size) == next)) {
        if (((char *)previous + BLOCK_SIZE + previous->block_size) == header) {
            printf("Stage 1: Merge left.\n");
            previous->block_size += BLOCK_SIZE + header->block_size; // block_size only contains the data section's size... Take into account overhead!
            if (((char *)previous + BLOCK_SIZE + previous->block_size) == next) {
                printf("Stage 2: Merge right.\n");
                previous->next_block = next->next_block;
                previous->block_size += BLOCK_SIZE + next->block_size;
            }
            printf("Head: %p\n", free_head);
            printf("Next: %p\n", free_head->next_block);
            if (free_head->next_block != NULL) {
                printf("Next Next: %p\n", free_head->next_block->next_block);
            }
            return;
        }
        if (((char *)header + BLOCK_SIZE + header->block_size) == next) {
            printf("Stage 1: Merge right.\n");
            printf("This should not happen with Stage 1: Merge left above!\n");
            header->block_size += BLOCK_SIZE + next->block_size;
            struct Block* temp = next->next_block;
            previous->next_block = header;
            header->next_block = temp;
